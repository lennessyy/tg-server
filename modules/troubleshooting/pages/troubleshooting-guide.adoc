...
Go down to the last error and it will point you to the error. This will show you any query errors that could be causing the failed installation. If you have created a xref:{page-component-version}@gsql-ref:querying:func/query-user-defined-functions.adoc[user-defined function], you could potentially have a c{pp} compilation error.
...
* Using quotes in the data file may cause interference with the tokenization of elements in the data file. Please check the GSQL Language Reference section under xref:{page-component-version}@gsql-ref:ddl-and-loading:creating-a-loading-job.adoc#_using_clause[USING Clause]. Look for the QUOTE parameter to see how you should set up your loading job.
...
This section will only cover the debugging schema change jobs, for more information about schema changes, please read the xref:{page-component-version}@gsql-ref:ddl-and-loading:modifying-a-graph-schema.adoc[Modifying a Graph Schema] page.
