...
If the system was handling a user request, the status code and message will be in the JSON response (see xref:gsql-ref:querying:query-operations.adoc#_gsql_query_output_format[GSQL Query Output Format] ).
For internal errors, the error information may be in a log file.
