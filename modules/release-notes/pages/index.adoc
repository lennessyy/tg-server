...
* Fixed issue with error codes in the log when a CDC message failed to deliver to external Kafka (CORE-4425).
...
* Fixed an exception (`NoSuchElementException`) when removing all tokens (GLE-9022).
...
* Added support for customizing pod labels and annotations in the `PodTemplateSpec` of the StatefulSet managing TigerGraph pods (TP-6901).
* Resolved a CDC `UNKNOWN` ID issue occurring under high load in the GSE service (CORE-4444).
