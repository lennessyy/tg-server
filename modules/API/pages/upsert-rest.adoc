...
Some edge types are defined with xref:gsql-ref:ddl-and-loading:defining-a-graph-schema.adoc#_discriminator[discriminators], which allow multiple instances of the same edge type between two vertices. Discriminators typically consist of one or more attributes that uniquely identify the edge.

To upsert an edge that was defined with a discriminator, follow these rules:

1. *Discriminators are required:*
When inserting an edge of a type defined with discriminator attributes, *you cannot omit the discriminator attributes*. For example, if the edge type includes `class_year` and `class_month` as discriminator attributes, both must be provided when inserting or updating the edge.

2. *Cannot update discriminator attributes:*
If you are updating an existing edge, *you cannot update the attributes that are part of the edge's discriminator*. These values are immutable once set.

=== How to upsert edges with discriminators

When upserting an edge with discriminators, you need to process the edge data into a *string format* that TigerGraph can accept. This is especially important when dealing with more complex edge structures, like the one shown below.

==== Example: Edge Definition

Suppose you have the following edge type definition:
...
When inserting an edge of type `Study_At`, you must include the `class_year` and `class_month` attributes in the JSON data.

==== Example JSON data (Before string conversion)
...
===== Formatted string for upsert

TigerGraph requires the edge data to be converted into a *string format* before performing the upsert operation. The JSON data above must be transformed into the following format:

[source.wrap,console]
----
'{"edges":{"Card":{"{\'1234-124-1245\': {\'Transacted_At\': {\'Merchant\': {\'TGT100\': {\'amount\': {\'value\': 99.95}, \'time\': {\'value\': 0}}}}}}":{"Transacted_At":{"Merchant":{"{\'TGT100\': {\'amount\': {\'value\': 99.95}, \'time\': {\'value\': 0}}}":{"amount": {"value": {"keylist": ["value"], "valuelist": [99.95]}}, "time": {"value": {"keylist": ["value"], "valuelist": [0]}}}}}}}}}}'
----

[NOTE]
====
Upserting edges with discriminators involves a non-standard process in TigerGraph’s REST API. 
You will need to convert the edge data into a specific string format (using *keylist/valuelist* pairs) for successful upsertion.
====
