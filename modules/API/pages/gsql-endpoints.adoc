...
[NOTE]
====
In 4.0, the paths for most endpoints were revised to follow a more standard convention.
Some functions were moved from the RESTPP server to the GSQL server for consistency.
You can refer to the xref:built-in-endpoints.adoc[REST endpoints for TigerGraph 3.x] for comparison.
====

NOTE: As of version 4.1.2, JWT tokens can be used for authentication with GSQL server REST endpoints.

.Sample schema
image::financialGraph_schema.png[]
