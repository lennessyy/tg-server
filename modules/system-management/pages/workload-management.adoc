...
The `GET WORKLOAD QUEUE` command retrieves the current workload queue configurations. This command is useful for exporting active configurations to modify them or back them up.
...
[NOTE]
====
* Use `GET WORKLOAD QUEUE` to dump queue configurations that are equivalent to the payload for POST.
* This is primarily an *administrative command* for exporting configurations.
* The `GET WORKLOAD QUEUE` command is a natural complement to other configuration commands like `PUT WORKLOAD QUEUE`.
* After using `GET WORKLOAD QUEUE` to retrieve current configurations, you can modify them and reapply using `PUT WORKLOAD QUEUE`.
====
...
* The `SHOW WORKLOAD QUEUE` command allows you to review queue permissions before applying any changes using `GRANT` or `REVOKE`.
====
